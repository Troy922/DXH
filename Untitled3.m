...
oxy_air=Newdata(:,68)/100;%空预器出口含氧量“%”
...
Q_hff=V_CH4*50016+V_H2*124238+V_CO*10077+V_oil*37000;
%挥发分放出热量KJ
...
Y=zeros(481,4);
C_o1=0.00002;
C_o2=(0.21*mean(Fg(577:716))/22.4-mean(C_bed(577:716))/12-mean(oxy_air(577:716).*Newdata(577:716,21)/(3600*22.4))-mean(Fhff_O2(577:716))- C_o1*Vb)/Vs;%稀相区氧气摩尔浓度kmol/m3
...
%    d3=0.21*Fg_x(i)/22.4;
%    d4=Fg_gas(i)/22.4;
   x=fsolve(@(x) myfun2(x,a1,a2,a3,a4,b1,b2,c1,c2,c3,d1),x0);
   Y(i-331,:)=x;
end
Y
y=mean(Y);
